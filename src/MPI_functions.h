                              ...
                              Shared_mem_float &shared_slots)
{
  ...
  shared_slots.ptr = (float*) malloc(sizeof(float) * size);
...
extern "C" int allocate_table(const int &num_slots, Shared_mem_int &shared_table)
{
  ...
  shared_table.ptr = (int*) malloc(sizeof(int) * size);
...
extern "C" int allocate_file_table(const int &num_files, Shared_mem_int &shared_file_table)
{
  ...
  shared_file_table.ptr = (int*) malloc(sizeof(int) * size);
...
extern "C" int open_files(const int &num_files, std::ifstream *files, Shared_mem_int &shared_file_table, std::string file_name)
{
  std::string full_name;
  for (int i=0; i<num_files; ++i){
    full_name = file_name + std::to_string(i);
    files[i].open(full_name, std::ios::in|std::ios::binary);
  }
}
...
extern "C" int finish(const int &rank, Shared_mem_int &shared_table, Shared_mem_float &shared_slots, Shared_mem_int &shared_file_table)
{
  ...
  free(shared_slots.ptr);
  free(shared_table.ptr);
  free(shared_file_table.ptr);
